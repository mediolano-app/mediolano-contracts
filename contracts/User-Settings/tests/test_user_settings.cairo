...
use snforge_std::{
    declare, ContractClassTrait, DeclareResultTrait, DeclareResult, start_cheat_caller_address, start_cheat_block_timestamp,
    stop_cheat_caller_address, stop_cheat_block_timestamp
};

use user_settings::interfaces::settings_interfaces::{IEncryptedPreferencesRegistryDispatcher, IEncryptedPreferencesRegistryDispatcherTrait};
...
fn deploy_contract() -> (IEncryptedPreferencesRegistryDispatcher, ContractAddress, ContractAddress) {
...
fn test_store_and_update_account_details() {
    ...
    dispatcher.store_account_details(
        owner_name, owner_email, username, current_timestamp //nonce, current_timestamp, version, pub_key, wallet_signature_arr
    );
    ...
    start_cheat_block_timestamp(this_contract, current_timestamp);
    start_cheat_caller_address(this_contract, owner);
    dispatcher.update_account_details(
        Option::Some('new name'), Option::None, Option::None, current_timestamp //2, current_timestamp, version, pub_key, new_wallet_signature_arr
    );
    stop_cheat_caller_address(this_contract);
    stop_cheat_block_timestamp(this_contract);
...
fn test_store_and_update_ip_management_settings() {
    let (dispatcher, this_contract, owner) = deploy_contract();
    // let (current_timestamp, version, pub_key, mut wallet_signature) = (400_u64, 1, 'pub_key', array!['s', 'r']);
    let current_timestamp = 400_u64;
    ...
    dispatcher.store_ip_management_settings(
        protection_level, automatic_ip_registration, current_timestamp //nonce, current_timestamp, version, pub_key, wallet_signature
    );
    let mut ip_management_settings = dispatcher.get_ip_settings(owner);
    stop_cheat_block_timestamp(this_contract);
    stop_cheat_caller_address(this_contract);
    
    assert(ip_management_settings.ip_protection_level == IPProtectionLevel::STANDARD, 'Store setting 1 failed');
    ...
    start_cheat_block_timestamp(this_contract, current_timestamp);
    start_cheat_caller_address(this_contract, owner);
    dispatcher.update_ip_management_settings(
        Option::Some(protection_level), Option::Some(automatic_ip_registration), current_timestamp //nonce, current_timestamp, version, pub_key, wallet_signature
    );
    stop_cheat_caller_address(this_contract);
    stop_cheat_block_timestamp(this_contract);
    ip_management_settings = dispatcher.get_ip_settings(owner);

    assert(ip_management_settings.ip_protection_level == IPProtectionLevel::ADVANCED, 'Store setting failed');
    assert(ip_management_settings.automatic_ip_registration == automatic_ip_registration, 'Store setting failed');    
}
...
fn test_store_and_update_notification_settings() {
    ...
    start_cheat_block_timestamp(this_contract, current_timestamp);
    start_cheat_caller_address(this_contract, owner);
    dispatcher.store_notification_settings(
        enable_notifications, ip_updates, blockchain_events, account_activity, current_timestamp //nonce, current_timestamp, version, pub_key, wallet_signature
    );
    stop_cheat_caller_address(this_contract);
    stop_cheat_block_timestamp(this_contract);
    ...
    start_cheat_block_timestamp(this_contract, current_timestamp);
    start_cheat_caller_address(this_contract, owner);
    dispatcher.update_notification_settings(
        Option::Some(enable_notifications), Option::Some(ip_updates), Option::Some(blockchain_events), Option::Some(account_activity), 
...
fn test_store_and_update_security_settings() {
    let (dispatcher, this_contract, owner) = deploy_contract();
    // let (current_timestamp, version, pub_key, mut wallet_signature, mut nonce) = (400_u64, 1, 'pub_key', array!['s', 'r'], 1);
    let current_timestamp = 400_u64;
    let mut password = 'password';

    let felt_caller: felt252 = owner.into();

    let hashed_password: felt252 = PoseidonTrait::new().update_with(password).update_with(current_timestamp).update_with(felt_caller).finalize().into();
    
    start_cheat_block_timestamp(this_contract, current_timestamp);
    start_cheat_caller_address(this_contract, owner);
    dispatcher.store_security_settings(
        password, current_timestamp //nonce, current_timestamp, version, pub_key, wallet_signature
    );
    ...
    start_cheat_block_timestamp(this_contract, current_timestamp);
    start_cheat_caller_address(this_contract, owner);
    dispatcher.update_security_settings(
        password, current_timestamp //nonce, current_timestamp, version, pub_key, wallet_signature
    );
    stop_cheat_caller_address(this_contract);
    stop_cheat_block_timestamp(this_contract);
    ...
    // These two will fail, simply because the hash of the password is what is stored, not the password
...
fn test_store_and_update_network_settings() {
    ...
    dispatcher.store_network_settings(
        network_type, gas_price_preference, current_timestamp //nonce, current_timestamp, version, pub_key, wallet_signature
    );
    stop_cheat_caller_address(this_contract);
    stop_cheat_block_timestamp(this_contract);
...
fn test_store_advanced_settings() {
    let (dispatcher, this_contract, owner) = deploy_contract();
    // let (current_timestamp, version, pub_key, mut wallet_signature, nonce) = (400_u64, 1, 'pub_key', array!['s', 'r'], 1);
    let current_timestamp = 400_u64;
    let api_key = 'api_key';
    ...
    start_cheat_block_timestamp(this_contract, current_timestamp);
    start_cheat_caller_address(this_contract, owner);
    dispatcher.store_advanced_settings(
        api_key, current_timestamp //nonce, current_timestamp, version, pub_key, wallet_signature
    );
    stop_cheat_caller_address(this_contract);
    stop_cheat_block_timestamp(this_contract);
...
fn test_store_x_verification() {
    ...
    start_cheat_block_timestamp(this_contract, current_timestamp);
    start_cheat_caller_address(this_contract, owner);
    dispatcher.store_X_verification(
        true, current_timestamp, x_handler //nonce, current_timestamp, version, pub_key, wallet_signature, x_handler
    );
    stop_cheat_caller_address(this_contract);
    stop_cheat_block_timestamp(this_contract);
