...
use ip_collective_agreement::interface::{
    ...
    IIPAssetManagerDispatcherTrait, IBerneComplianceDispatcher, IBerneComplianceDispatcherTrait, IRevenueDistributionDispatcher,
    IRevenueDistributionDispatcherTrait, ILicenseManagerDispatcher, ILicenseManagerDispatcherTrait, IGovernanceDispatcher, IGovernanceDispatcherTrait
};
...
use super::test_utils::{
    ...
    create_test_creators_data, register_test_asset, deploy_erc1155_receiver, setup_with_compliance, register_test_authority, create_and_approve_verification, array_contains
...
fn test_register_compliance_authority() {
    let (contract_address, _, asset_dispatcher, _, _, _, _, compliance_dispatcher, _, owner_address) = setup_with_compliance();
    let authority_address = deploy_erc1155_receiver();
    let authorized_countries = array!['US', 'CA', 'MX'].span();
    
    start_cheat_caller_address(contract_address, owner_address);
    let success = compliance_dispatcher.register_compliance_authority(
        authority_address,
    ...
    assert!(compliance_dispatcher.is_authorized_for_country(authority_address, 'US'), "Should be authorized for US");
    assert!(compliance_dispatcher.is_authorized_for_country(authority_address, 'CA'), "Should be authorized for CA");
    assert!(!compliance_dispatcher.is_authorized_for_country(authority_address, 'UK'), "Should not be authorized for UK");
}
...
fn test_register_authority_unauthorized() {
    ...
    compliance_dispatcher.register_compliance_authority(
        authority_address, "Unauthorized Authority", array!['US'].span(), 'GOVERNMENT', "ipfs://fake"
    );
}
...
fn test_register_authority_invalid_type() {
    let (contract_address, _, _, _, _, _, _, compliance_dispatcher, _, owner_address) = setup_with_compliance();
    let authority_address = deploy_erc1155_receiver();
    
    start_cheat_caller_address(contract_address, owner_address);
    compliance_dispatcher.register_compliance_authority(
        authority_address, "Invalid Authority", array!['US'].span(), 'INVALID_TYPE', "ipfs://creds"
    );
}
...
fn test_deactivate_compliance_authority() {
    let (contract_address, _, _, _, _, _, _, compliance_dispatcher, _, owner_address) = setup_with_compliance();
    let authority_address = register_test_authority(contract_address, compliance_dispatcher, owner_address);
    
    // Verify authority is initially active
    assert!(compliance_dispatcher.is_authorized_for_country(authority_address, 'US'), "Should be authorized initially");
    ...
    assert!(!compliance_dispatcher.is_authorized_for_country(authority_address, 'US'), "Should not be authorized after deactivation");
    
    let authority = compliance_dispatcher.get_compliance_authority(authority_address);
    assert!(!authority.is_active, "Authority should be inactive");
}

// ========== COUNTRY REQUIREMENTS TESTS ==========
...
fn test_request_compliance_verification() {
    let (contract_address, _, asset_dispatcher, _, _, _, _, compliance_dispatcher, _, owner_address) = setup_with_compliance();
    let (asset_id, creators, _, _) = register_test_asset(contract_address, asset_dispatcher, owner_address);
    ...
    assert!(request.requested_status == ComplianceStatus::BerneCompliant.into(), "Wrong requested status");
...
fn test_request_verification_unauthorized() {
    let (contract_address, _, asset_dispatcher, _, _, _, _, compliance_dispatcher, _, owner_address) = setup_with_compliance();
    let (asset_id, _, _, _) = register_test_asset(contract_address, asset_dispatcher, owner_address);
    let non_owner = USER();
    
    start_cheat_caller_address(contract_address, non_owner);
    compliance_dispatcher.request_compliance_verification(
        asset_id, ComplianceStatus::BerneCompliant.into(), "ipfs://fake", 
        'US', 1640995200, WorkType::Musical.into(), true, array![non_owner].span()
    );
}
...
fn test_process_compliance_verification_success() {
    ...
    let request_id = compliance_dispatcher.request_compliance_verification(
        ...
        'US', 1640995200, WorkType::Musical.into(), true, array![creator1, creator2].span()
    );
    stop_cheat_caller_address(contract_address);
    ...
    let success = compliance_dispatcher.process_compliance_verification(
        request_id,
        true,
    ...
    assert!(updated_request.verifier_notes == "Meets all Berne Convention requirements", "Wrong verifier notes");
    ...
    assert!(compliance_record.compliance_status == ComplianceStatus::BerneCompliant.into(), "Wrong compliance status");
    assert!(compliance_record.country_of_origin == 'US', "Wrong country of origin");
    assert!(compliance_record.registration_authority == authority_address, "Wrong registration authority");
    assert!(compliance_record.protection_duration == protection_duration, "Wrong protection duration");
    ...
    assert!(compliance_record.automatic_protection_count == 3, "Should have 3 auto-protected countries");
    assert!(compliance_record.manual_registration_count == 2, "Should have 2 manual registration countries");
    
    // Verify asset compliance status was updated
    let asset_info = asset_dispatcher.get_asset_info(asset_id);
    assert!(asset_info.compliance_status == ComplianceStatus::BerneCompliant.into(), "Asset status should be updated");
...
fn test_process_verification_unauthorized() {
    ...
    let request_id = compliance_dispatcher.request_compliance_verification(
        asset_id, ComplianceStatus::BerneCompliant.into(), "ipfs://evidence",
        'US', 1640995200, WorkType::Musical.into(), true, array![creator1].span()
    );
    stop_cheat_caller_address(contract_address);
...
fn test_check_protection_validity() {
    ...
    assert!(compliance_dispatcher.check_protection_validity(asset_id, 'US'), "Should have protection in US");
    
    // Should not have protection in non-registered country
    assert!(!compliance_dispatcher.check_protection_validity(asset_id, 'JP'), "Should not have protection in JP");
    
    // Register international protection for JP
    start_cheat_caller_address(contract_address, *creators[0]);
    compliance_dispatcher.register_international_protection(
        asset_id, array!['JP'].span(), array!["ipfs://jp-registration"].span()
    );
    stop_cheat_caller_address(contract_address);
    
    // Now should have protection in JP
    assert!(compliance_dispatcher.check_protection_validity(asset_id, 'JP'), "Should have protection in JP after registration");
}
...
fn test_protection_duration_calculation() {
    ...
    let duration = compliance_dispatcher.calculate_protection_duration(
        'US', WorkType::Musical.into(), 1640995200, false
    );
    ...
    let anonymous_duration = compliance_dispatcher.calculate_protection_duration(
        'US', WorkType::Musical.into(), 1640995200, true
    );
...
fn test_renewal_requirements() {
    ...
    create_and_approve_verification(contract_address, compliance_dispatcher, asset_id, creator1, authority_address);
...
fn test_validate_license_compliance() {
    ...
    create_and_approve_verification(contract_address, compliance_dispatcher, asset_id, *creators[0], authority_address);
    
    // Should be valid for US territory
    let is_valid = compliance_dispatcher.validate_license_compliance(
        asset_id, 'US', 'US', 'COMMERCIAL'
    );
    assert!(is_valid, "License should be valid for protected territory");
    
    // Should not be valid for unprotected territory
    let is_invalid = compliance_dispatcher.validate_license_compliance(
        asset_id, 'JP', 'JP', 'COMMERCIAL'
    );
...
fn test_get_licensing_restrictions() {
    ...
    let us_reqs = CountryComplianceRequirements {
        country_code: 'US', is_berne_signatory: true, automatic_protection: true,
        registration_required: false, protection_duration_years: 95, notice_required: true,
        deposit_required: false, translation_rights_duration: 7, moral_rights_protected: false,
    };
...
fn test_mark_protection_expired() {
    ...
    assert!(initial_record.compliance_status == ComplianceStatus::BerneCompliant.into(), "Should be initially compliant");
    ...
    assert!(expired_record.compliance_status == ComplianceStatus::NonCompliant.into(), "Should be non-compliant after expiry");
    
    // Asset info should also be updated
    let asset_info = asset_dispatcher.get_asset_info(asset_id);
    assert!(asset_info.compliance_status == ComplianceStatus::NonCompliant.into(), "Asset status should be updated");
}
...
fn test_public_domain_detection() {
    ...
    // Create verification with very short protection (1 second for testing)
    start_cheat_caller_address(contract_address, *creators[0]);
    let request_id = compliance_dispatcher.request_compliance_verification(
        asset_id, ComplianceStatus::BerneCompliant.into(), "ipfs://evidence",
        'US', 50,
        WorkType::Musical.into(), true, array![*creators[0]].span()
    );
    stop_cheat_caller_address(contract_address);
    
    // Approve with very short protection (1 second)
    start_cheat_caller_address(contract_address, authority_address);
    compliance_dispatcher.process_compliance_verification(
        request_id, true, "Short protection for testing", 5, // 5 second protection
        array!['US'].span(), array![].span()
    );
    stop_cheat_caller_address(contract_address);
