...
use core::integer::{u64, u256}; // Import required integer types
...
use ip_launchpad::crowd_funding::Crowdfunding::{
 AssetCreated, Funded, FundingClosed, CreatorWithdrawal, InvestorWithdrawal
};
// Traits derived from the contract interface, allowing interaction with a deployed contract
use ip_launchpad::interfaces::ICrowdfunding::{ICrowdfundingDispatcher, ICrowdfundingDispatcherTrait};
// Note: Assuming the interface ICrowdfunding and its dispatcher ICrowdfundingDispatcher are defined in the parent module of the Crowdfunding contract module.
...
fn deploy_crowdfunding_contract(
    ...
    // 1. Declare the contract class, identified by its module name "Crowdfunding" <a href="https://book.cairo-lang.org/ch104-02-testing-smart-contracts.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">6</a>
    let contract = declare("Crowdfunding");

    let erc20_dispatcher = deploy_mock_erc20(owner);

    // 2. Create constructor arguments - serialize each one into a felt252 array <a href="https://book.cairo-lang.org/ch104-02-testing-smart-contracts.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">6</a>
    ...
    // 3. Deploy the contract and retrieve its address <a href="https://book.cairo-lang.org/ch104-02-testing-smart-contracts.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">6</a>
    ...
    // 4. Create a dispatcher to interact with the contract using its interface <a href="https://book.cairo-lang.org/ch104-02-testing-smart-contracts.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">6</a>
...
fn test_constructor() {
    ...
    assert!(token_addr == token_dispatcher.contract_address, "Initial token_address not set correctly");
...
fn test_create_asset() {
    ...
    // Setup cheatcodes: mock caller and block timestamp <a href="https://book.cairo-lang.org/ch104-02-testing-smart-contracts.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">6</a><a href="https://foundry-rs.github.io/starknet-foundry/testing/testing-contract-internals.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">10</a>
    ...
    // Setup event spy to capture emitted events <a href="https://book.cairo-lang.org/ch104-02-testing-smart-contracts.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">6</a><a href="https://foundry-rs.github.io/starknet-foundry/testing/testing-contract-internals.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">10</a>
    ...
    assert!(stored_ipfs_hash.len() == ipfs_hash_parts.len(), "Stored IPFS hash length mismatch");
    ...
    // Verify event emission <a href="https://book.cairo-lang.org/ch104-02-testing-smart-contracts.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">6</a><a href="https://foundry-rs.github.io/starknet-foundry/testing/testing-contract-internals.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">10</a>
...
fn test_fund_success() {
    ...
    let expected_event = Crowdfunding::Event::Funded(
        Funded { asset_id: asset_id, investor: investor, amount: amount_to_fund, timestamp: fund_time }
    );
    let expected_events = array![(dispatcher.contract_address, expected_event)];
    spy.assert_emitted(@expected_events);
}
...
fn test_fund_panic_zero_amount() {
    ...
    start_cheat_block_timestamp(dispatcher.contract_address, 1000 + 100); // Within funding period
...
fn test_fund_panic_before_start() {
    ...
    start_cheat_caller_address(dispatcher.contract_address, INVESTOR_ADDRESS_1);
    start_cheat_block_timestamp(dispatcher.contract_address, start_time - 1); // Before funding period
...
fn test_fund_panic_after_end() {
    ...
    start_cheat_caller_address(dispatcher.contract_address, INVESTOR_ADDRESS_1);
    start_cheat_block_timestamp(dispatcher.contract_address, start_time + duration + 1); // After funding period
    dispatcher.fund(0, 100.into());
    stop_cheat_caller_address(dispatcher.contract_address);
    stop_cheat_block_timestamp(dispatcher.contract_address);
}
...
fn test_fund_panic_insufficient_funds() {
    ...
    start_cheat_caller_address(dispatcher.contract_address, INVESTOR_ADDRESS_1);
    start_cheat_block_timestamp(dispatcher.contract_address, start_time + 100); // Within funding period
...
fn test_close_funding_success() {
    ...
    let investor = INVESTOR_ADDRESS_1;
    let amount_to_fund: u256 = 100.into(); // Fund enough to meet or exceed goal (discounted price 90)
    start_cheat_caller_address(dispatcher.contract_address, investor);
    start_cheat_block_timestamp(dispatcher.contract_address, start_time + 10); // Within funding period
...
fn test_close_funding_panic_before_end() {
    ...
    start_cheat_caller_address(dispatcher.contract_address, CREATOR_ADDRESS);
    start_cheat_block_timestamp(dispatcher.contract_address, 1000 + 50); // Within funding period
    dispatcher.close_funding(0);
    stop_cheat_caller_address(dispatcher.contract_address);
    stop_cheat_block_timestamp(dispatcher.contract_address);
}
...
fn test_withdraw_creator_success() {
    ...
    let expected_event = Crowdfunding::Event::CreatorWithdrawal(
        CreatorWithdrawal { asset_id: 0, amount: amount_to_fund } // Amount should be total raised
    );
    let expected_events = array![(dispatcher.contract_address, expected_event)];
    spy.assert_emitted(@expected_events);
...
fn test_withdraw_investor_success() {
    ...
    assert!(investor_data_before.amount == amount_to_fund, "Investment amount incorrect before withdrawal");
    ...
    assert!(investor_data_after.amount == 0.into(), "Investment amount not reset after withdrawal");
...
fn test_set_token_address_success() {
    ...
    // Verify state update using load (since no public getter for token_address) <a href="https://book.cairo-lang.org/ch104-02-testing-smart-contracts.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">6</a>
...
fn test_set_token_address_panic_not_owner() {
    ...
    // Set caller address to a non-owner address <a href="https://book.cairo-lang.org/ch104-02-testing-smart-contracts.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">6</a><a href="https://foundry-rs.github.io/starknet-foundry/testing/testing-contract-internals.html" target="_blank" rel="noopener noreferrer" className="bg-light-secondary dark:bg-dark-secondary px-1 rounded ml-1 no-underline text-xs text-black/70 dark:text-white/70 relative hover:underline">10</a>
    start_cheat_caller_address(dispatcher.contract_address, OTHER_ADDRESS);
...
fn test_getters() {
    ...
    assert!(stored_ipfs_hash.len() == ipfs_hash_parts.len(), "get_asset_ipfs_hash: Length mismatch");
    let mut i: usize = 0;
    while i != ipfs_hash_parts.len() {
        assert!(*stored_ipfs_hash.at(i) == *ipfs_hash_parts.at(i), "get_asset_ipfs_hash: Part mismatch");
        i += 1;
    };
    ...
    assert!(non_existent_investor_data.amount == 0.into(), "get_investor_data: Non-existent investor amount not zero");
    assert!(non_existent_investor_data.timestamp == 0, "get_investor_data: Non-existent investor timestamp not zero");
}
