...
pub mod IPTokenizer {
    ...
    use super::super::types::{
        ...
        INVALID_LICENSE_TERMS, DEFAULT_BATCH_LIMIT, ERROR_EMPTY_BATCH, ERROR_BATCH_TOO_LARGE,
    };
    ...
    impl IPTokenizerImpl of IIPTokenizer<ContractState> {
        ...
        fn update_metadata(ref self: ContractState, token_id: u256, new_metadata: ByteArray) {
            ...
            let updated_asset = IPAssetData { metadata_uri: new_metadata, ..asset };
            self.tokens.write(token_id, updated_asset);
        }

        fn update_license_terms(ref self: ContractState, token_id: u256, new_terms: LicenseTerms) {
            self.ownable.assert_only_owner();
            let mut asset = self.tokens.read(token_id);
    ...
    impl Private of PrivateTrait {
        fn _mint(ref self: ContractState, asset: IPAssetData) -> u256 {
            ...
            assert(
                self._validate_license_terms(asset.clone().license_terms), INVALID_LICENSE_TERMS
            );
