...
use ip_subscription::Subscription::Subscription::{
    PlanCreated, Subscribed, SubscriptionRenewed, SubscriptionUpgraded, Unsubscribed,
};
...
fn test_create_plan() {
    ...
    let plan_id = dispatcher.create_plan(price, duration, tier);

    // Verify event emission
    let expected_event = Subscription::Subscription::Event::PlanCreated(
        PlanCreated { plan_id, price: price, duration: duration, tier: tier },
    );
...
fn test_subscribe() {
    ...
    assert(is_subscribed, 'Should be subscribed');

    // Verify event emission
    let expected_event = Subscription::Subscription::Event::Subscribed(
        Subscribed { subscriber: subscriber, plan_id },
    );
    let expected_events = array![(dispatcher.contract_address, expected_event)];
    spy.assert_emitted(@expected_events);
...
fn test_unsubscribe() {
    ...
    // Unsubscribe
    start_cheat_caller_address(dispatcher.contract_address, subscriber);
    dispatcher.unsubscribe(plan_id);

    // Verify subscription status
    let is_subscribed = dispatcher.get_subscription_status();
    assert(!is_subscribed, 'Should not be subscribed');

    // Verify event emission
    let expected_event = Subscription::Subscription::Event::Unsubscribed(
        Unsubscribed { subscriber: subscriber, plan_id },
    );
    let expected_events = array![(dispatcher.contract_address, expected_event)];
    spy.assert_emitted(@expected_events);
...
fn test_upgrade_subscription() {
    ...
    start_cheat_caller_address(dispatcher.contract_address, owner);
    let plan_id_1 = dispatcher.create_plan(price_1, duration, tier_1);
    let plan_id_2 = dispatcher.create_plan(price_2, duration, tier_2);
    stop_cheat_caller_address(dispatcher.contract_address);
...
fn test_create_plan_not_owner() {
    ...
    dispatcher.create_plan(price, duration, tier);
    stop_cheat_caller_address(dispatcher.contract_address);
}
...
#[test]
#[should_panic(expected: 'Not subscribed to this plan')]
